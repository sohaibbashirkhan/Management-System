...
$get_purchase_query = "SELECT `product_id`, `quantity` FROM `lowitempurchase` WHERE `id` = $id";
...
$update_product_query = "UPDATE `lowitemproduct` SET `quantity` = `quantity` - $quantity WHERE `id` = $productid";
...
$query = "DELETE FROM `lowitempurchase` WHERE `id` = $id";
...
?>
<script>
    window.location.assign("lowpurchase.php");
</script>
