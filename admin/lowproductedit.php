...
if(isset($_GET['id'])) {
    $id = $_GET['id'];
    $query = "SELECT * FROM lowproductitem WHERE id = ?";
...
if(isset($_POST['submit'])) {
    ...
    // if(move_uploaded_file($temp_name, $upload_dir . $picture)) {
        ...
        $query = "UPDATE lowitemproduct SET title=?, color=?, pack=?, quantity=?, price=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $query);
        ...
        if(mysqli_stmt_execute($stmt)) {
            ...
            header("Location: lowproduct.php");
...
<head>
    ...
    <title>Dashboard - Low Product Edit</title>
        ...
        <main>
            <div class="container-fluid px-4">
                ...
                <ol class="breadcrumb mb-4">
                    <li class="breadcrumb-item active">Edit Low Product Dashboard</li>
                </ol>
                <form action="lowproductupdate.php?id=<?= $row['id'] ?>" method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" value="<?= $row['color'] ?>" name="txtcolor" class="form-control"  >
                    </div>
                    <div class="mb-3">
                        <label for="" class="form-label">Pack</label>
                        <select name="txtpack" class="form-control" required >
                            <option value="">Select Pack</option>
                            <option value="Kg" <?php if($row['Kg'] == "Kg") echo "selected"; ?>>Kg</option>
                            <option value="Liter" <?php if($row['Liter'] == "Liter") echo "selected"; ?>>Liter</option>
                            <option value="Number" <?php if($row['Number'] == "Number") echo "selected"; ?>>Number</option>
                        </select>
                    </div>
                    <div class="mb-3">
