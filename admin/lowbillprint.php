        ...
        class BillPrinter {
            ...
            public function printBill() {
                if(isset($_POST["submit"])) {
                    ...
                    $select = "SELECT lowitempurchase.id, lowitempurchase.Customer_name, lowitemproduct.title, lowitempurchase.quantity, lowitempurchase.date, lowitempurchase.price  
                        FROM `lowitempurchase` 
                        INNER JOIN `lowitemproduct` ON lowitempurchase.product_id = lowitemproduct.id
                        WHERE lowitempurchase.id = $customer AND date BETWEEN '$startdate' AND '$enddate'";

                    $count = "SELECT SUM(lowitempurchase.price * lowitempurchase.quantity) 
                                FROM `lowitempurchase` 
                                WHERE lowitempurchase.id = $customer AND date BETWEEN '$startdate' AND '$enddate'";
                    ...
                    if(mysqli_num_rows($result)) {
                        $user = "SELECT * FROM `lowitempurchase` WHERE `id` = $customer";
