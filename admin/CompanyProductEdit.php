...
if(isset($_GET['id'])) {
    $id = $_GET['id'];
    $query = "SELECT * FROM companyproduct WHERE id = ?";
...
if(isset($_POST['submit'])) {
    ...
    // if(move_uploaded_file($temp_name, $upload_dir . $picture)) {
        ...
        $query = "UPDATE companyproduct SET title=?, color=?, pack=?, quantity=?, price=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $query);
...
<head>
    ...
    <title>Dashboard - Company Product Edit</title>
    ...
    <div id="layoutSidenav_content">
        <main>
            <div class="container-fluid px-4">
                <h1 class="mt-4">Edit Company product</h1>
                <ol class="breadcrumb mb-4">
                    <li class="breadcrumb-item active">Edit product Dashboard</li>
                </ol>
                <form action="companyproductupdate.php?id=<?= $row['id'] ?>" method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
