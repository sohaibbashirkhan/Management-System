        ...
        class BillPrinter {
            ...
            public function printBill() {
                if(isset($_POST["submit"])) {
                    ...
                    $select = "SELECT purchase.id, purchase.Customer_name, product.title, purchase.quantity, purchase.date, purchase.price  
                        FROM `purchase` 
                        INNER JOIN `product` ON purchase.product_id = product.id
                        WHERE purchase.id = $customer AND date BETWEEN '$startdate' AND '$enddate'";

                    $count = "SELECT SUM(purchase.price * purchase.quantity) 
                                FROM `purchase` 
                                WHERE purchase.id = $customer AND date BETWEEN '$startdate' AND '$enddate'";
                    ...
                    if(mysqli_num_rows($result)) {
                        $user = "SELECT * FROM `purchase` WHERE `id` = $customer";
