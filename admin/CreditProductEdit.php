...
if(isset($_GET['id'])) {
    $id = $_GET['id'];
    $query = "SELECT * FROM creditproduct WHERE id = ?";
...
if(isset($_POST['submit'])) {
    ...
    // if(move_uploaded_file($temp_name, $upload_dir . $picture)) {
        ...
        $query = "UPDATE creditproduct SET title=?, color=?, pack=?, quantity=?, price=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "sssssi", $Title, $Color, $Pack, $Quantity, $Price, $id);
        if(mysqli_stmt_execute($stmt)) {
            mysqli_stmt_close($stmt);
            // Redirect to companycompanyproduct.php after successful update
            header("Location: CompanyProduct.php");
...
<head>
    ...
    <title>Dashboard - Credit Product Edit</title>
...
<div id="layoutSidenav_content">
    <main>
        <div class="container-fluid px-4">
            <h1 class="mt-4">Edit Credit Product</h1>
            <ol class="breadcrumb mb-4">
                <li class="breadcrumb-item active">Edit Credit Product Dashboard</li>
            </ol>
            ...
            <form action="" method="POST">
                ...
                <!--    <input type="file" id="txtpicture" name="txtpicture" class="form-control" required>-->
                <!--</div>-->
