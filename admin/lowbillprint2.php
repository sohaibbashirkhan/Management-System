        ...
        class BillPrinter {
            ...
            public function printBill() {
                if(isset($_POST["submit"])) {
                    ...
                    $select = "SELECT lowitemorder.id, lowitemorder.Customer_name, lowitemproduct.title, lowitemorder.quantity, lowitemorder.date, lowitemorder.price  
                        FROM `lowitemorder` 
                        INNER JOIN `lowitemproduct` ON lowitemorder.product_id = lowitemproduct.id
                        WHERE lowitemorder.id = $customer AND date BETWEEN '$startdate' AND '$enddate'";

                    $count = "SELECT SUM(lowitemorder.price * lowitemorder.quantity) 
                                FROM `lowitemorder` 
                                WHERE lowitemorder.id = $customer AND date BETWEEN '$startdate' AND '$enddate'";
                    ...
                    if(mysqli_num_rows($result)) {
                        $user = "SELECT * FROM `orders` WHERE `id` = $customer";
